<?php
    function GetInsData($conn) {
        $stmt = $conn->query("SELECT * FROM ins");
        $stmt->execute();
        $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $data;
    }
    function GetInsByID($id, $conn) {
        $stmt = $conn->query("SELECT * FROM ins WHERE id = '$id'");
        $stmt->execute();
        $data = $stmt->fetch(PDO::FETCH_ASSOC);
        return $data;
    }
    function GetNameInsByID($id, $conn) {
        $stmt = $conn->query("SELECT ins FROM ins WHERE id = $id");
        $stmt->execute();
        $data = $stmt->fetch(PDO::FETCH_ASSOC);
        return $data['ins'];
    }
    ...
    function GetInsEx($id, $conn) {
        $stmt = $conn->query("SELECT id, ins FROM ins WHERE id <> $id");
        $stmt->execute();
